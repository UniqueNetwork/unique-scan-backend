...
class Logger {
  ...
  static getTimestamp() {
    ...
    const month = addLeadZero(d.getMonth() + 1);
    const date = addLeadZero(d.getDate());
    const hour = addLeadZero(d.getHours());
    const min = addLeadZero(d.getMinutes());
    const sec = addLeadZero(d.getSeconds());
    return `${year}-${month}-${date} ${hour}:${min}:${sec}`;
  }

  static formatString(str, fmt) {
    const fmtSymbol = this.getFmtSymbol(
      this.getFmt(typeof fmt === 'undefined' ? '' : fmt),
    );
  ...
  constructLog(message, level) {
    ...
    let msgs =
      level !== this.level.NONE
        ? [
            `${
              this.constructor.LEVEL_COLOR.hasOwnProperty(level)
                ? this.fmt(level, this.constructor.LEVEL_COLOR[level])
                : level
            }:`,
          ]
        : [];
    ...
    if (this.includeTime) {
      msgs = [
        `[${this.fmt(this.constructor.getTimestamp(), 'fg.cyan')}]`,
        ...msgs,
      ];
    }
    return msgs;
